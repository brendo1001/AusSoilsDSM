...
if(machineName=='TERNSOIL2'){
  basePath <- '//fs1-cbr.nexus.csiro.au/{af-tern-mal-deb}/work/Ross/New/Auscover/FinalMosaics'
  templateR <- raster('//fs1-cbr.nexus.csiro.au/{af-tern-mal-deb}/work//datasets/national/covariates/mosaics/30m/Masked/mask30.tif')
}else{
  basePath <- '/datasets/work/af-tern-mal-deb/work/Ross/New/Auscover/FinalMosaics'
...
outDir <- paste0(basePath, '/Resampled')
...
r2 <- resample(inR, templateR, filename=paste0(outDir, '/',fls[k] ), method = 'ngb')
