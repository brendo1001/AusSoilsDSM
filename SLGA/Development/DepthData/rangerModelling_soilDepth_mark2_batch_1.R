...
#data
# observations
obs_dat<- readRDS("/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/data/sd_siteDat_covariates_fin.rds")
str(obs_dat)
...
## 5 models for job
for (i in 1:5){  # colum 5 is where the target variable info begins
  ...
  brC<- c(rbeta(len_censoredDat, shape1 = 2, shape2 =4))
  ...
  #fit model
  ranger.model<-train(x= DSM_data_c[,c(4:35)], y= DSM_data_c$simDat,
                        tuneGrid = tgrid, 
                        method = "ranger",
  ...
  calOut_dat<- DSM_data_c[,c(1,2,3,36:38)]
  ...
  valOut_dat<- DSM_data_v[,c(1,2,3,36:38)]
