...
models<- list.files(path = "/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/models/", pattern = ".rds",full.names = TRUE)
models
...
oper1<- foreach(i=1:250, .packages = c("raster", "sp", "rgdal", "caret", "ranger")) %dopar% {
  ...
  nmz<- paste0("/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/rcode/slurm/slurmCheck/Iteration_counter_", batch, "_", i, ".txt")
  write.table(itOuts, 
              file = nmz,
