...
avail.datasets<- fromJSON("http://esoil.io/TERNLandscapes/SoilDataFederatoR/R/SoilDataAPI/DataSets?usr=ross.searle%40csiro.au&key=a")

lodfs <- vector("list", length(avail.datasets$DataSet))
for (i in 1:length(avail.datasets$DataSet)){
  nm1<- paste0("https://esoil.io/TERNLandscapes/SoilDataFederatoR/R/SoilDataAPI/SoilData?observedProperty=O_PPF;O_GSG;O_ASC_ORD;O_ASC_SUBORD&DataSet=",avail.datasets$DataSet[i],"&usr=brendan.malone%40csiro.au&key=djwjgrpt74ld7wm")
  df <- fromJSON(nm1)
  print(head(df))
  lodfs[[i]] <- df
}
